...
export default function Home() {
  ...
  const currentBrief = chapterBriefs[current.n];
  ...
  const copyBrief = async () => {
    ...
    await navigator.clipboard.writeText(currentBrief.map((s) => `${s.label}\n${s.text}`).join("\n\n"));
    setBriefCopied(true);
            ...
            {currentBrief && (
              <section className="five-minute-brief" aria-labelledby="five-minute-title">
                <header>
                  ...
                  <div><span>ORAL BRIEFING · CHAPTER {String(current.n).padStart(2, "0")}</span><h3 id="five-minute-title">5分鐘精華版</h3><p>可直接口述的現場報告稿 · 約五分鐘</p></div>
                  <button className="brief-toggle" onClick={() => setBriefOpen(!briefOpen)} aria-expanded={briefOpen} aria-controls="chapter-brief">{briefOpen ? "收合 −" : "展開 +"}</button>
                </header>
                {briefOpen && <div id="chapter-brief" className="brief-content">
                  <div className="brief-toolbar"><p><b>講述主線</b> 核心問題 → 重要概念 → 原住民族語研究 → 阿美語研究缺口</p><button onClick={copyBrief}>{briefCopied ? "✓ 已複製" : "複製口述稿"}</button></div>
                  <ol>{currentBrief.map((segment) => <li key={segment.time}><div className="brief-time"><span>{segment.time}</span><i aria-hidden="true" /></div><div><h4>{segment.label}</h4><p>{segment.text}</p></div></li>)}</ol>
